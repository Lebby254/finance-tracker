    ...
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 7:
          ...
          topCategory = sortedCategories.length > 0 ? sortedCategories[0][0] : "N/A"; // --- TREND ANALYSIS ---
          ...
          healthScore = 0; // 1️⃣ Savings Ratio (0–40)

          savingsRatio = totalIncome > 0 ? balance / totalIncome * 100 : 0;
          healthScore += Math.max(0, Math.min(40, savingsRatio / 100 * 40)); // 2️⃣ Trend Factor (0–40)

          if (trend === "improving") healthScore += 40;else if (trend === "stable") healthScore += 25;else if (trend === "declining") healthScore += 10; // 3️⃣ Expense Control (0–20)

          expenseRatio = totalIncome > 0 ? totalExpense / totalIncome * 100 : 100;
          if (expenseRatio <= 70) healthScore += 20;else if (expenseRatio <= 90) healthScore += 10;else healthScore += 5; // Clamp 0–100
